<div class="container-fluid mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3"><i class="fas fa-edit"></i> Edit KPI Template</h1>
    ...
    <div class="card shadow">
        ...
        <div class="card-body">
            <form method="post" action="<?= base_url('admin/edit_template_group') ?>" id="templateForm">
                <!-- Hidden field with original template name -->
                <input type="hidden" name="original_template_name" value="<?= htmlspecialchars($template_name) ?>">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="template_name" name="template_name" 
                           ...
                           value="<?= htmlspecialchars($template_name) ?>" required>
                        ...
                        <?php if (isset($departments) && !empty($departments)): ?>
                            <?php foreach ($departments as $dept): ?>
                                <option value="<?= $dept->department_id ?>" 
                                        <?= (isset($current_department_id) && $current_department_id == $dept->department_id) ? 'selected' : '' ?>>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5><i class="fas fa-tasks"></i> KPI Items (<?= count($kpis) ?> total)</h5>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="kpiTable">
                        ...
                        <tbody id="kpiTableBody">
                            ...
                            <tr class="kpi-row">
                                <td>
                                    <textarea class="form-control" name="kpis[<?= $index ?>][kpi_name]" 
                                              rows="2" placeholder="Enter KPI task name" required><?= htmlspecialchars($kpi->kpi_name) ?></textarea>
                                </td>
                                <td>
                                    <select class="form-control" name="kpis[<?= $index ?>][category_id]" required>
                                        <option value="">-- Select --</option>
                                        <?php foreach ($categories as $category): ?>
                                            <option value="<?= $category->category_id ?>" 
                                                    <?= (isset($kpi->category_id) && $kpi->category_id == $category->category_id) ? 'selected' : '' ?>>
                                ...
                                <td>
                                    <textarea class="form-control" name="kpis[<?= $index ?>][description]" 
                                              rows="2" placeholder="Optional details"><?= isset($kpi->description) ? htmlspecialchars($kpi->description) : '' ?></textarea>
                                </td>
                ...
                <div class="alert alert-info">
                    ...
                    <ul class="mb-0 mt-2">
                        <li><strong>Weightage will be assigned by Manager</strong> when assigning KPIs to employees</li>
                        <li>Changes will affect future template assignments (not existing employee KPIs)</li>
                        <li>You can add or remove KPIs from this template</li>
                    </ul>
                </div>
